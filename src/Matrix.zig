...
pub fn multiplyByVec4(self: *const Matrix, vec4: Vec4) Vec4 {
    return Vec4{ .x = self.m[0][0] * vec4.x + self.m[0][1] * vec4.y + self.m[0][2] * vec4.z + self.m[0][3] * vec4.w, .y = self.m[1][0] * vec4.x + self.m[1][1] * vec4.y + self.m[1][2] * vec4.z + self.m[1][3] * vec4.w, .z = self.m[2][0] * vec4.x + self.m[2][1] * vec4.y + self.m[2][2] * vec4.z + self.m[2][3] * vec4.w, .w = self.m[3][0] * vec4.x + self.m[3][1] * vec4.y + self.m[3][2] * vec4.z + self.m[3][3] * vec4.w };
}
